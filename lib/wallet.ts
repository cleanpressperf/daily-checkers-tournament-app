...
export function canAfford(amount: number) { return Number.isFinite(amount) && amount > 0 && getBalance() >= amount }
export function addBalance(amount: number) { const next = getBalance() + Math.max(0, Math.floor(amount)); setBalance(next); return next }
export const addCoins = addBalance
export function deductBalance(amount: number) { if (!canAfford(amount)) return false; setBalance(getBalance() - amount); return true }
