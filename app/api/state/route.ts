  ...
  if(caps.length===0&&moves.length===0){
    ...
    if(newWinners.length>=s.bracket.length/2){
      if(s.roundIdx===4 && newWinners.length===1){
        ...
        let used=s.usedPool||[...ALL_96].sort(()=>Math.random()-0.5); let idx=s.poolIndex||0; if(idx+32>used.length){ used=[...ALL_96].sort(()=>Math.random()-0.5); idx=0; }
...
function initMemory(){
  const shuffled=[...ALL_96].sort(()=>Math.random()-0.5); const first32=shuffled.slice(0,32);
...
export async function GET(){
  try{
    ...
    const elapsed=Date.now()-MEMORY.time;
    let steps=Math.floor(elapsed/4000);
    if(steps>0){
      // max 5 steps per request to avoid huge jump = no drag, smooth worldwide
      steps=Math.min(steps,1);
      for(let i=0;i<steps;i++){
        MEMORY=simulateOne(MEMORY);
        if(MEMORY._pendingNext){
          const nxt=MEMORY._pendingNext; MEMORY={...nxt, time:Date.now()};
        }
