...
export default function WatchPage(){
  ...
  const [move,setMove]=useState(0); const [last,setLast]=useState(""); const [winner,setWinner]=useState<string|null>(null); const [champion,setChampion]=useState<string|null>(null);
  ...
  const audioRef=useRef<AudioContext|null>(null); const chainCountRef=useRef(0);
  ...
  const save = (data:any={})=>{
    const state = { tournamentNum, roundIdx, matchInRound, bracket, nextRoundPlayers, roundWinners, p1, p2, board, turn, move, last, winner, champion, chainPos, time: Date.now(),...data };
  ...
  // LOAD + CATCH-UP + LIVE POLL
  useEffect(()=>{
    fetch('/api/state').then(r=>r.json()).then(s=>{
      if(s && s.bracket?.length){
        if(s.champion && Date.now()-(s.time||0) > CHAMP_TIME){ const r32=getRandom32(); setTournamentNum((s.tournamentNum||1)+1); setBracket(r32); setNextRoundPlayers([]); setRoundWinners([]); setRoundIdx(0); setMatchInRound(0); startMatch(r32[0],r32[1]); setLoaded(true); save({tournamentNum:(s.tournamentNum||1)+1,bracket:r32,roundIdx:0,matchInRound:0,nextRoundPlayers:[],roundWinners:[],champion:null}); return; }
        setTournamentNum(s.tournamentNum||1); setRoundIdx(s.roundIdx||0); setMatchInRound(s.matchInRound||0); setBracket(s.bracket); setNextRoundPlayers(s.nextRoundPlayers||[]); setRoundWinners(s.roundWinners||[]); setP1(s.p1); setP2(s.p2); setBoard(s.board); setTurn(s.turn||1); setMove(s.move||0); setLast(s.last||""); setChampion(s.champion||null); setChainPos(s.chainPos||null); setLoaded(true);
      } else { const r32=getRandom32(); setBracket(r32); setTournamentNum(1); setRoundIdx(0); setMatchInRound(0); setNextRoundPlayers([]); setRoundWinners([]); startMatch(r32[0],r32[1]); setLoaded(true); save({bracket:r32,tournamentNum:1,roundIdx:0,matchInRound:0}); }
    }).catch(()=>{ const r32=getRandom32(); setBracket(r32); startMatch(r32[0],r32[1]); setLoaded(true); });
  },[]);

  useEffect(()=>{ if(loaded) save(); },[tournamentNum,roundIdx,matchInRound,bracket,nextRoundPlayers,roundWinners,p1,p2,turn,move,champion,loaded]);
  ...
  useEffect(()=>{
    ...
    const id=setTimeout(()=>{
      setBoard(prev=>{
        ...
        bd[chosen.nr][chosen.nc]=moving; bd[chosen.r][chosen.c]=0; if(chosen.capture){ bd[chosen.capture.r][chosen.capture.c]=0; if(chainPos) sounds.doubleCapture(); else sounds.capture(); const more=getCaptures(bd,chosen.nr,chosen.nc); if(more.length>0) setChainPos({r:chosen.nr,c:chosen.nc}); else { setChainPos(null); setTurn(t=>t===1?2:1); } } else { sounds.move(); setChainPos(null); setTurn(t=>t===1?2:1); }
        ...
        setMove(m=>m+1); setLast(chosen.capture?`CAPTURE`:`→ ${chosen.nc},${chosen.nr}`);
        return bd;
      });
    },delay); return()=>clearTimeout(id);
  },[board,turn,chainPos,winner,champion,loaded,soundOn,p1,p2]);

  useEffect(()=>{ if(!winner) return; sounds.win(); speak(`Winner ${winner} advances`); const newWinners=[...roundWinners, winner]; setRoundWinners(newWinners); const newNext=[...nextRoundPlayers, winner]; setNextRoundPlayers(newNext); if(roundIdx===4){ setChampion(winner); speak(`Champion ${winner}`); sounds.champ(); save({champion:winner,roundWinners:newWinners,nextRoundPlayers:newNext}); return; } const matchesInRound=bracket.length/2; const nextMatch=matchInRound+1; setTimeout(()=>{ if(nextMatch < matchesInRound){ setMatchInRound(nextMatch); startMatch(bracket[nextMatch*2], bracket[nextMatch*2+1]); save({matchInRound:nextMatch,roundWinners:newWinners,nextRoundPlayers:newNext}); } else { setBracket(newNext); setNextRoundPlayers([]); setRoundWinners([]); setRoundIdx(roundIdx+1); setMatchInRound(0); startMatch(newNext[0], newNext[1]); save({bracket:newNext,nextRoundPlayers:[],roundWinners:[],roundIdx:roundIdx+1,matchInRound:0}); } }, BETWEEN_GAMES); },[winner]);

  useEffect(()=>{ if(!champion) return; const t=setTimeout(()=>{ const r32=getRandom32(); setTournamentNum(tournamentNum+1); setBracket(r32); setNextRoundPlayers([]); setRoundWinners([]); setRoundIdx(0); setMatchInRound(0); setChampion(null); startMatch(r32[0], r32[1]); fetch('/api/state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tournamentNum:tournamentNum+1,bracket:r32,nextRoundPlayers:[],roundWinners:[],roundIdx:0,matchInRound:0,champion:null,p1:r32[0],p2:r32[1],board:newBoard(),time:Date.now()})}); }, CHAMP_TIME); return()=>clearTimeout(t); },[champion]);

  if(!loaded) return <div style={{background:"#000",color:"#fff",padding:20}}>Loading tournament...</div>;
    ...
    <div style={{background:"#000",minHeight:"100vh",color:"#fff",padding:16}}>
      ...
      <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
        ...
        <div style={{minWidth:200,background:"#111",borderRadius:12,padding:12,height:"fit-content"}}>
          ...
          <div style={{fontSize:10,opacity:0.4,marginTop:10}}>Live synced via KV — all phones see same board</div>
        </div>
      </div>
    </div>
  )
