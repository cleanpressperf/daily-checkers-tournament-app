...
export default function Watch(){
  ...
  useEffect(()=>{
    ...
    let st:any={bracket:sh.slice(0,32),p1:sh[0],p2:sh[1],board:newBoard(),turn:RNG()>0.5?1:2,roundIdx:0,matchInRound:0,roundWinners:[],chain:null,finalCelebration:false};
    ...
    const iv=setInterval(()=>{
      ...
      if(s.finalCelebration){
        const elapsed=Math.floor((Date.now()-s.finalTime)/1000); const remaining=300-elapsed;
        if(remaining<=0){ const sh2=shuffleFisher(ALL_96,rngRef.current); stateRef.current={bracket:sh2.slice(0,32),p1:sh2[0],p2:sh2[1],board:newBoard(),turn:rngRef.current()>0.5?1:2,roundIdx:0,matchInRound:0,roundWinners:[],chain:null,finalCelebration:false}; lastWinnerRef.current=""; speak("New tournament beginning now!"); }
        else { setInfo({...s, countdown:remaining}); draw(s.board); return; }
      }
