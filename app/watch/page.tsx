...
export default function Watch(){
  ...
  useEffect(()=>{
    ...
    const fastSync=async()=>{
      ...
      if(d?.bracket?.length){
        setS((prev:any)=>{
          if(!prev) return d;
          if(d.time===prev.time &&!d._pendingNext) return prev;
          return d;
        });
      }
    };
    ...
    const poll=setInterval(fastSync, 1000);
  ...
  return <div style={{background:"#000",color:"#fff",minHeight:"100vh",padding:12,position:"relative"}}>
    ...
    <div style={{color:"#ff3b3b",fontSize:11}}>◎ 24/7 INTL LIVE · {ROUND_NAMES[s.roundIdx]||"FINISHED"} · Game {s.matchInRound+1}/{Math.ceil(s.bracket.length/2)} 🔊</div>
    ...
    <div style={{fontSize:9,opacity:0.25,marginTop:8,textAlign:"center"}}>Locked board • instant live • no replay drag • back capture + flying king + longest mandatory • 4s move 1s chain • 🔊 voice</div>
  </div>;
}
