...
// NEW START = TODAY 15:30 GMT - so first load is only ~100 moves, instant on any phone
const GLOBAL_START = 1758574200000;
...
export default function Watch(){
  ...
  useEffect(()=>{
    ...
    // FAST FORWARD but in chunks to not freeze UI
    let i=0;
    const chunk=()=>{
      const end=Math.min(target, i+30);
      for(;i<end;i++){ st=simulateOne(st,RNG); }
      moveRef.current=i;
      stateRef.current=st;
      if(i<target){ setTimeout(chunk,0); } else { draw(st.board); setInfo({...st,move:i}); startLive(); }
    };
    ...
    const startLive=()=>{
      setInterval(()=>{
        ...
        setInfo({...stateRef.current,move:moveRef.current});
      },4000);
    };
    const onResize=()=>{ if(stateRef.current) draw(stateRef.current.board); };
    window.addEventListener("resize",onResize);
    chunk();
    return()=>window.removeEventListener("resize",onResize);
  },[]);
  if(!info) return <div style={{background:"#000",color:"#fff",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}><div>Syncing global game...</div><div style={{fontSize:12,opacity:0.5,marginTop:8}}>Loading {Math.floor((Date.now()-GLOBAL_START)/4000)} moves fast</div></div>;
  return <div style={{background:"#000",color:"#fff",minHeight:"100vh",padding:12,display:"flex",flexDirection:"column",alignItems:"center"}}>
    <div style={{width:"100%",maxWidth:520}}>
      <div style={{color:"#ff3b3b",fontSize:12,fontWeight:700}}>◎ 24/7 LIVE GLOBAL · {ROUND_NAMES[info.roundIdx]} · G{info.matchInRound+1} · M{info.move}</div>
      <h2 style={{margin:"8px 0"}}>{info.p1} <span style={{color:"#666"}}>vs</span> {info.p2}</h2>
      ...
      <div style={{marginTop:10,fontSize:11,opacity:0.6}}>Same board on all devices · Canvas fixed the rainbow glitch</div>
    </div>
  </div>;
}
