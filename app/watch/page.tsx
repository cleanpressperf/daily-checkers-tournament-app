...
export default function Watch(){
  ...
  useEffect(()=>{
    let iv:any;
    const init=async()=>{
      const info=await fetch('/api/state?t='+Date.now(),{cache:'no-store'}).then(r=>r.json()).catch(()=>({start:1758537600000, seed:123456, bracket:ALL_96.slice(0,32)}));
      startRef.current=info.start||1758537600000;
      rngRef.current=mulberry32(info.seed||123456);
      const shuffled=[...ALL_96].sort(()=>rngRef.current()-0.5);
      const first32=info.bracket||shuffled.slice(0,32);
      ...
      for(let i=0;i<target;i++){ state=simulateOne(state, rngRef.current); if(i>10000) break; }
      stateRef.current=state;
      moveRef.current=target;
      setS({...state, move:target});

      iv=setInterval(()=>{
        const elapsed2=Date.now()-startRef.current;
        const target2=Math.floor(elapsed2/4000);
        while(moveRef.current < target2){
          ...
          if(moveRef.current>15000) break;
        }
  ...
  return <div style={{background:"#000",color:"#fff",minHeight:"100vh",padding:10,display:"flex",flexDirection:"column",alignItems:"center"}}>
    <div style={{width:"100%",maxWidth:560}}>
      <div style={{color:"#ff3b3b",fontSize:12,fontWeight:700}}>◎ 24/7 LIVE LOCAL ENGINE · {ROUND_NAMES[s.roundIdx]} · Game {s.matchInRound+1}/{Math.ceil(s.bracket.length/2)} · Move {s.move}</div>
      ...
      <div style={{width:"100%",aspectRatio:"1/1",background:"#3d2814",border:"4px solid #5a3e2b",borderRadius:16,overflow:"hidden",display:"grid",gridTemplateColumns:"repeat(10,1fr)",gridTemplateRows:"repeat(10,1fr)"}}>
        {s.board.map((row:any,r:number)=>row.map((cell:any,c:number)=>{
          ...
          return <div key={r+"-"+c} style={{background:isLight?"#f0d9b5":"#b58863",display:"flex",alignItems:"center",justifyContent:"center"}}>
            {cell!==0&&<div style={{width:"84%",height:"84%",borderRadius:"50%",background:cell===1||cell===11?"radial-gradient(circle at 30% 30%,#444,#000)":"radial-gradient(circle at 30% 30%,#ff5555,#900)",border:cell===11||cell===22?"3px solid #ffd700":"1.5px solid #000",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      <div style={{textAlign:"center",fontSize:10,opacity:0.3,marginTop:12}}>Local engine • no back-forth • never freezes • perfect size • sound on tap</div>
    </div>
  </div>;
}
