...
export default function Watch(){
  ...
  // FIXED SYNC - instant on tab focus / return
  useEffect(()=>{
    const init=async()=>{ if(creatingRef.current) return; creatingRef.current=true; let d=await fetch('/api/state').then(r=>r.json()).catch(()=>null);
      if(!d?.bracket?.length){ const shuffled=[...ALL_96].sort(()=>0.5-Math.random()); const first32=shuffled.slice(0,32); d={bracket:first32,p1:first32[0],p2:first32[1],board:newBoard(),turn:1,roundIdx:0,matchInRound:0,roundWinners:[],champion:null,time:Date.now(),chain:null,usedPool:shuffled,poolIndex:32}; await fetch('/api/state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)}); }
      setS(d); creatingRef.current=false; setFlash(true); setTimeout(()=>setFlash(false),600);
    };
    init();
    const fastSync=async()=>{ const d=await fetch('/api/state?t='+Date.now()).then(r=>r.json()).catch(()=>null); if(d?.bracket?.length){ if(JSON.stringify(d)!==JSON.stringify(s)){ setS(d); setFlash(true); setTimeout(()=>setFlash(false),500);} } };
    const onVisible=()=>{ if(document.visibilityState==='visible'){ fastSync(); } };
    document.addEventListener('visibilitychange', onVisible);
    window.addEventListener('focus', fastSync);
    const poll=setInterval(fastSync, 1500);
  ...
  useEffect(()=>{
    ...
    timerRef.current=setTimeout(async()=>{
      ...
      if(caps.length===0&&moves.length===0){
        ...
        } else { next.matchInRound=s.matchInRound+1; next.roundWinners=newWinners; next.p1=s.bracket[next.matchInRound*2]; next.p2=s.bracket[next.matchInRound*2+1]; next.board=newBoard(); next.turn=1; playSound('win'); speak(`${matchWinner} wins, proceeding`); }
  ...
  if(!s) return <div style={{background:"#000",color:"#fff",padding:30}}>Loading...</div>;
  return <div style={{background:"#000",color:"#fff",minHeight:"100vh",padding:12,position:"relative"}}>
    ...
    <div style={{color:flash?"#0f0":"#ff3b3b",fontSize:11,transition:"color 0.3s"}}>{flash?"⚡ SYNCED TO LIVE":"◎ 24/7 INTL LIVE"} · {ROUND_NAMES[s.roundIdx]||"FINISHED"} · Game {s.matchInRound+1}/{Math.ceil(s.bracket.length/2)} {s.chain?`· CHAIN`:""} 🔊</div>
    ...
    {/* LOCKED GRID - NO EXPAND */}
    <div style={{width:"100%",maxWidth:400,aspectRatio:"1/1",border:flash?"3px solid #0f0":"3px solid #5a3e2b",borderRadius:12,overflow:"hidden",margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(10, minmax(0,1fr))",gridTemplateRows:"repeat(10, minmax(0,1fr))",transition:"border 0.3s"}}>
      {s.board.map((row:any,r:number)=>row.map((cell:any,c:number)=>{
        const isLight=(r+c)%2===0;
        return <div key={r+"-"+c} style={{background:isLight?"#f5deb3":"#8b5a2b",width:"100%",height:"100%",minWidth:0,minHeight:0,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",boxSizing:"border-box",overflow:"hidden"}}>
          {cell!==0&&<div style={{width:"72%",height:"72%",minWidth:0,minHeight:0,borderRadius:"50%",background:cell===1||cell===11?"#111":"#c00",border:cell===11||cell===22?"2px solid gold":"1px solid rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1,flexShrink:0,boxSizing:"border-box"}}>
            <span style={{fontSize:"9px",lineHeight:1,display:"block",color:"gold",fontWeight:900,transform:"scale(0.9)"}}>{cell===11||cell===22?"♔":""}</span>
          </div>}
        </div>;
      }))}
    </div>
    ...
    <div style={{marginTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
      <div style={{background:"#111",border:"1px solid #333",borderRadius:8,padding:8,flex:1,minWidth:200}}>
        ...
        <div style={{fontSize:11,marginTop:4}}>{s.roundWinners.length? s.roundWinners.map((n:string,i:number)=><span key={i} style={{background:"#222",padding:"2px 6px",borderRadius:4,margin:"2px",display:"inline-block"}}>{n}</span>): <span style={{opacity:0.3}}>— winners add here —</span>}</div>
      </div>
    </div>
    <div style={{fontSize:9,opacity:0.25,marginTop:8,textAlign:"center"}}>Fixed king size • instant sync on tab switch • back capture ✓ • 4s/1s • sound 🔊</div>
  </div>;
}
