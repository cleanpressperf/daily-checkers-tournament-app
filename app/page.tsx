...
export default function Page(){
 const [sec,setSec]=useState(0); const [sj,setSj]=useState(false); const [tournaments, setTournaments] = useState<any[]>([
 ...
 useEffect(()=>{
  async function load(){
    ...
    if(tours){
      ...
      if(withCounts.length) setTournaments(withCounts)
    }
  }
  load();
 ...
 return (<main className="min-h-screen bg-black text-white"><Nav tick={tick}/><section className="mx-auto max-w-7xl px-5 py-16 lg:px-10"><div className="rounded-[28px] border border-white/10 bg-zinc-950 p-6"><div className="flex justify-between text-sm text-zinc-400"><span>🔴 LIVE NOW - Bots playing</span><Clock3 className="size-4"/></div><div className="mt-5 text-6xl tabular-nums">{h}:{m}:{s}</div><p className="mt-2 text-sm text-zinc-400">32/32 bots auto-filled • Click eye to watch</p></div><div className="mt-10 grid gap-4 md:grid-cols-3">{tournaments.map(t=><Card key={t.name} t={t} onJoined={(n)=>{setJn(n);setSj(true);setTick(x=>x+1)}} onWatch={()=>setSn(true)}/>)}</div></section>{sn&&<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-5"><div className="w-full max-w-sm rounded-[24px] bg-zinc-900 p-6 text-center"><p className="text-xs tracking-[0.2em] text-[#ffd700]">🔴 LIVE NOW</p><h3 className="mt-3 text-xl font-semibold">Bots are playing!</h3><p className="mt-2 text-sm text-zinc-400">{tournaments.map(tt=>`${tt.name}: ${tt.joined}/${tt.max_players}`).join(' • ')}</p><button onClick={()=>{setSn(false); window.location.href='/play/demo-match'}} className="mt-6 w-full rounded-xl bg-white py-3 font-semibold text-black">Watch Live</button><button onClick={()=>setSn(false)} className="mt-3 w-full rounded-xl bg-zinc-800 py-3 font-semibold text-white">Close</button></div></div>}{sj&&<JoinedModal name={jn} close={()=>{setSj(false); window.location.href='/play/demo-match'}}/>}</main>)
}
