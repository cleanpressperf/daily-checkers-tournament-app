...
function useWATCountdown() {
  const [seconds, setSeconds] = useState(0)
  useEffect(() => { const update = () => { const now = new Date(); const parts = new Intl.DateTimeFormat('en-US', { timeZone: 'Africa/Lagos', hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' }).formatToParts(now); const h = Number(parts.find(p => p.type === 'hour')?.value || 0), m = Number(parts.find(p => p.type === 'minute')?.value || 0), s = Number(parts.find(p => p.type === 'second')?.value || 0); setSeconds((19 * 3600 - (h * 3600 + m * 60 + s) + 86400) % 86400) }; update(); const timer = setInterval(update, 1000); return () => clearInterval(timer) }, [])
  return `${Math.floor(seconds / 3600)}h ${Math.floor((seconds % 3600) / 60).toString().padStart(2, '0')}m ${(seconds % 60).toString().padStart(2, '0')}s`
}

export default function TournamentsPage() {
  const countdown = useWATCountdown()
