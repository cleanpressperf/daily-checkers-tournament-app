...
const SIZE=10; type Cell=0|1|2|3|4; type Move = { toR:number, toC:number, capR:number, capC:number, isCap:boolean };
function createBoard(): Cell[][]{ const b: Cell[][]=Array(SIZE).fill(null).map(()=>Array(SIZE).fill(0)); for(let r=0;r<4;r++) for(let c=0;c<SIZE;c++) if((r+c)%2===1) b[r][c]=1; for(let r=6;r<10;r++) for(let c=0;c<SIZE;c++) if((r+c)%2===1) b[r][c]=2; return b; }
function getCaps(board:Cell[][], r:number,c:number, player:1|2){ const caps:any[]=[]; const isKing=board[r][c]===3||board[r][c]===4; const enemy=player===1?[2,4]:[1,3]; const dirs=[[1,1],[1,-1],[-1,1],[-1,-1]]; for(const [dr,dc] of dirs){ if(isKing){ let mr=r+dr,mc=c+dc; while(mr>=0&&mr<SIZE&&mc>=0&&mc<SIZE){ if(board[mr][mc]!==0){ if(enemy.includes(board[mr][mc])){ let tr=mr+dr,tc=mc+dc; while(tr>=0&&tr<SIZE&&tc>=0&&tc<SIZE&&board[tr][tc]===0){ caps.push({toR:tr,toC:tc,capR:mr,capC:mc}); tr+=dr; tc+=dc; } } break; } mr+=dr; mc+=dc; } }else{ const mr=r+dr,mc=c+dc,tr=r+dr*2,tc=c+dc*2; if(mr>=0&&mr<SIZE&&mc>=0&&mc<SIZE&&tr>=0&&tr<SIZE&&tc>=0&&tc<SIZE){ if(enemy.includes(board[mr][mc]) && board[tr][tc]===0){ caps.push({toR:tr,toC:tc,capR:mr,capC:mc}); } } } } return caps; }
function getSimples(board:Cell[][], r:number,c:number, player:1|2){ const moves:any[]=[]; const isKing=board[r][c]===3||board[r][c]===4; if(isKing){ const dirs=[[1,1],[1,-1],[-1,1],[-1,-1]]; for(const [dr,dc] of dirs){ let nr=r+dr,nc=c+dc; while(nr>=0&&nr<SIZE&&nc>=0&&nc<SIZE&&board[nr][nc]===0){ moves.push({toR:nr,toC:nc}); nr+=dr; nc+=dc; } } }else{ const dirs=player===1?[[1,1],[1,-1]]:[[-1,1],[-1,-1]]; for(const [dr,dc] of dirs){ const nr=r+dr,nc=c+dc; if(nr>=0&&nr<SIZE&&nc>=0&&nc<SIZE&&board[nr][nc]===0) moves.push({toR:nr,toC:nc}); } } return moves; }
function countPieces(board:Cell[][], player:1|2){ let cnt=0; for(let r=0;r<SIZE;r++) for(let c=0;c<SIZE;c++){ if(player===1 && (board[r][c]===1||board[r][c]===3)) cnt++; if(player===2 && (board[r][c]===2||board[r][c]===4)) cnt++; } return cnt; }
function hasAnyMove(board:Cell[][], player:1|2){ for(let r=0;r<SIZE;r++) for(let c=0;c<SIZE;c++){ if((player===1&&(board[r][c]===1||board[r][c]===3))||(player===2&&(board[r][c]===2||board[r][c]===4))){ if(getCaps(board,r,c,player).length>0) return true; if(getSimples(board,r,c,player).length>0) return true; } } return false; }
...
function PlayInner(){
  ...
  const [turn,setTurn]=useState<1|2>(2); const [sel,setSel]=useState<any>(null);
  const [moves,setMoves]=useState<Move[]>([]); const [timer,setTimer]=useState(30);
  const [status,setStatus]=useState("Your turn"); const [won,setWon]=useState(false);
  const timerRef=useRef<any>(null);

  // push live to backend + localStorage for watch
  ...
  useEffect(()=>{ pushLive(board, turn, status); },[board,turn]);
  ...
  useEffect(()=>{
    ...
    if(botCount===0){ handleWin(); } else if(humanCount===0){ alert("You lost!"); window.location.href=`/watch?t=${tParam}`; }
    else if(turn===1 &&!hasAnyMove(board,1)){ handleWin(); } else if(turn===2 &&!hasAnyMove(board,2)){ alert("No moves! You lost."); window.location.href=`/watch?t=${tParam}`; }
  },[board,turn]);
  ...
  function handleWin(){
    ...
    try{
      ...
      if(saved?.state){ const st=saved.state; if(!st.roundWinners.includes(me)){ st.roundWinners=[...st.roundWinners, me]; if(st.roundWinners.length>=st.bracket.length/2 && st.roundIdx<4){ st.bracket=[...st.roundWinners]; st.roundIdx+=1; st.matchInRound=0; st.roundWinners=[]; } localStorage.setItem(SAVE_KEY, JSON.stringify({state:st, ticks:Math.floor(Date.now()/4000)})); } }
      localStorage.setItem(`qualified_${tParam}`, JSON.stringify({name:me, time:Date.now()}));
      // clear live
    ...
    setTimeout(()=>{ alert(`🏆 ${me} WINS! Proceeding to next round. Go to WATCH now. Come back before next round ends or DQ.`); router.push(`/watch?t=${tParam}`); },1200);
  }

  function onSelect(r:number,c:number){
    if(won||turn!==2) return; const piece=board[r][c]; if(piece===2||piece===4){
  ...
  return (
    <div style={{background:"#0f0f0f", minHeight:"100vh", color:"#fff", padding:8}}>
      <div style={{maxWidth:420, margin:"0 auto"}}>
        ...
        <div style={{marginTop:6, fontSize:11, color:"#FFD700"}}>{status} • 🔴 LIVE for spectators</div>
        ...
        <div style={{marginTop:6, fontSize:9, color:"#999", textAlign:"center"}}>RED= capture (backward allowed) • GREEN= move • You win when opponent 0 pieces or no moves • Auto to WATCH</div>
