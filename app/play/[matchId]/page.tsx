...
export default function LiveMatchPage({ params }: { params: Promise<{ matchId: string }> }) {
  ...
  return <main className="min-h-screen bg-black p-5 text-white">
    ...
    <div className="mx-auto max-w-5xl py-10">
      ...
      <h1 className="mt-3 text-3xl font-bold">{match?.player1_name || 'Connecting...'} vs {match?.player2_name || 'Connecting...'}</h1>
      <p className="mt-2 text-zinc-400">Table {match?.table_number || '—'} · {match?.round || '—'} · {error ? 'connecting' : match?.status || 'connecting'}</p>
